...
global config_path
config_path = r'E:\projects\pycharm\weather_product\Background\docs\LyhxConfig.ini'
global section_ftp
section_ftp = 'FTP'
...
class LyhxService:
    ...
    def upload_files(self):
        ...
        if self.check_file(local_dir):
            ...
            count = 0
            for f in files:
                if '.txt' == os.path.splitext(f)[1] and 14 == len(f):
                    local_path = local_dir + f
                    fileDate = '20' + f[4:10]
                    now = datetime.date.today()
                    time_str = datetime.datetime.strftime(now, '%Y%m%d')
                    if time_str == fileDate:
                        new_filename = 'NMF_TRF_OSTR_CSDT_' + fileDate + '00_072h_OCE.txt'
        ...
        print('旅游航线预报总共上传' + str(count) + '个文件')
...
def scheduleTask():
    ...
    scheduler.add_job(task, "cron", coalesce=True, misfire_grace_time=3600, day_of_week="0-6", hour=19, minute=30)
...
def task():
    lyhxService = LyhxService(config_path, section_ftp, section_local)

    lyhxService.upload_files()
